...
# We intentionally do not impose a dictionary byte limit.
#
# The dictionary should contain useful vocabulary first.
# The APK build will tell us the real compressed size.
#
# The delete indexes ARE budgeted because they can grow very
# quickly and are mainly useful for correction.
...
# Frequency source entries available for selection.
#
...
# The dictionary is ordered by frequency.
#
# The most frequent words get edit-distance 2 deletes.
# The remaining words get distance 1 deletes.

TOP_DISTANCE2_WORDS=15000

MAX_DELETE_DISTANCE=2

# Long words remain in the dictionary.
# This limit only controls the delete index.
MAX_DELETE_WORD_LENGTH=24

# Limit candidate explosion per delete key.
...
# IMPORTANT:
#
# The old script extracted only the base entries from .dic.
#
# That means frequency words such as:
#
#   podés
#   tenés
#   hacés
#
# could be rejected if they were generated by Hunspell affix
# rules rather than present literally in the .dic file.
#
# unmunch expands the actual Hunspell word forms.
#
# This can be considerably slower.
# That is intentional.
# ============================================================
...
expand_hunspell() {
    ...
    echo ""
    echo "============================================================"
    echo " Expanding Hunspell word forms: ${language}"
    echo "============================================================"
...
print(
    f"Expanded Hunspell forms: {len(seen)}"
)
PY

    if [[ ! -s "${output}" ]]; then
        echo "ERROR: Hunspell expansion produced no words:"
        echo "  ${language}"
        exit 1
    fi

    echo "Expanded forms: $(wc -l < "${output}")"
}
...
expand_hunspell \
    ...
    "${ES_DIR}/index.dic" \
    "${ES_DIR}/index.aff" \
    "${ES_DIR}/hunspell.expanded"

expand_hunspell \
    "en-en" \
    "${EN_DIR}/index.dic" \
    "${EN_DIR}/index.aff" \
    "${EN_DIR}/hunspell.expanded"

expand_hunspell \
    "de-de" \
    "${DE_DIR}/index.dic" \
    "${DE_DIR}/index.aff" \
    "${DE_DIR}/hunspell.expanded"
...
# Priority:
#
#   1. Frequency-ranked vocabulary validated by Hunspell
#   2. Remaining Hunspell-generated forms
#
# There is intentionally NO manual whitelist.
#
# The order is important because the delete generator uses
# dictionary order as frequency priority.
# ============================================================
...
def valid_word(word):
    ...
    # Keep Unicode letters, apostrophes and hyphens.
    #
    # We deliberately do not impose a short dictionary-word
    # limit. Long words may still be useful.
    for char in word:
        if char.isalpha():
            continue
...
# ------------------------------------------------------------
#
# Add every valid Hunspell form not already represented by
# the frequency source.
#
# This is what prevents the frequency corpus from becoming
# the only vocabulary gate.
# ------------------------------------------------------------
...
# ============================================================
#
# These checks are diagnostics only.
#
# They NEVER add words and NEVER affect the build.
# ============================================================
